...
def create_vehicle(vehicle: VehicleBase, session: Session = Depends(get_session)):
...
def update_vehicle(vehicle_id: int, updated_vehicle: VehicleBase, session: Session = Depends(get_session)):
    vehicle = session.get(Vehicle, vehicle_id)
    if not vehicle:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Vehicle not found")
...
def partial_update_vehicle(vehicle_id: int, vehicle_update: VehiclePartialUpdate, session: Session = Depends(get_session)):
    vehicle = session.get(Vehicle, vehicle_id)
    if not vehicle:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Vehicle not found")
