...
def create_equipment_safety(equipment_safety: EquipmentSafetyBase, session: Session = Depends(get_session)):
...
def update_equipment_safety(equipment_safety_id: int, updated_equipment_safety: EquipmentSafetyBase, session: Session = Depends(get_session)):
    equipment_safety = session.get(EquipmentSafety, equipment_safety_id)
    if not equipment_safety:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Equipment Safety not found")
...
def partial_update_equipment_safety(equipment_safety_id: int, equipment_safety_update: EquipmentSafetyPartialUpdate, session: Session = Depends(get_session)):
    equipment_safety = session.get(EquipmentSafety, equipment_safety_id)
    if not equipment_safety:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Equipment Safety not found")
...
def delete_equipment_safety(equipment_safety_id: int, session: Session = Depends(get_session)):
    equipment_safety = session.get(EquipmentSafety, equipment_safety_id)
    if not equipment_safety:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Equipment Safety not found")
